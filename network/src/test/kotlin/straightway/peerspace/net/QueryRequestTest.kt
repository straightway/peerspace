...
class QueryRequestTest {
    ...
    private companion object {
        const val matchedId = "matchedId"
        const val unmatchedId = "unmatchedId"
    }

    @Test
    fun testAutoGeneratedDataClassMethods() {
        val sut = QueryRequest(matchedId, untimedData)
        sut.testAutoGeneratedDataClassMethods()
    }

    @Test
    fun `matches key with given id and zero timestamp`() =
            Given { QueryRequest(matchedId, untimedData) } when_ {
    ...
    fun `matches timestamp within range`() =
            Given { QueryRequest(matchedId, 1L..2L) } when_ {
    ...
    fun `onlyMostRecent flag can be set by static creator method`() =
            Given { QueryRequest.onlyMostRecent(matchedId) } when_ {
                onlyMostRecent
            } then {
                expect(it.result is_ True)
            }
    ...
    fun `onlyMostRecent with timestamp has a range starting at the timestamp`() =
            Given { QueryRequest.onlyMostRecent(matchedId, 83L) } when_ {
                timestamps
            } then {
