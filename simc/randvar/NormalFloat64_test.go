...
type NormalFloat64_Test struct {
	suite.Suite
	randSource rand.Source
}

func TestNormalFloat64(t *testing.T) {
	suite.Run(t, new(NormalFloat64_Test))
}

func (suite *NormalFloat64_Test) SetupTest() {
	suite.randSource = rand.NewSource(12345)
}

// Tests

func (suite *NormalFloat64_Test) TestMeanValueIsAsSpecified() {
	mean := 4.0
	stdDeviation := 2.0
	sut := NewNormalFloat64(suite.randSource, mean, stdDeviation)
...
func determineMeanAndStdDeviation(randVar *NormalFloat64, numSamples int, definedMean float64) (mean, stdDev float64) {
	sum := float64(0.0)
...
func (suite *NormalFloat64_Test) assertDeviationBelow(expected, actual float64, maxDeviation float64) {
